    ...
    for m in [1, 2, 3, 5]:
        for k in [1, 2, 4, 6]:
            ...
            problems.append(_problem(
                ...
                difficulty="tier1", system="spring",
                coordinates="x",
            ))
    return problems
...
def _tier1_freefall():
    ...
    for m in [1, 2, 3, 5]:
        ...
        problems.append(_problem(
            ...
            difficulty="tier1", system="freefall",
            coordinates="y",
        ))
    return problems
    ...
    for m in [1, 2]:
        for F in [3, 5, 10]:
            ...
            problems.append(_problem(
                ...
                dqdt_exprs=[sp.diff(H, p)],
                dpdt_exprs=[-sp.diff(H, x)],
                difficulty="tier1", system="constant_force",
                coordinates="x",
            ))
    return problems
    ...
    for a in [1, 2, 3]:
        for b in [1, 3, 5]:
            ...
            problems.append(_problem(
                ...
                dqdt_exprs=[sp.diff(H, p)],
                dpdt_exprs=[-sp.diff(H, x)],
                difficulty="tier2", system="duffing",
                coordinates="x",
            ))
    return problems
...
def _tier3_coupled_oscillators():
    ...
    for k1 in [1, 2, 4]:
        for kc in [1, 3]:
            m = 1
            T = p1**2 / (2 * m) + p2**2 / (2 * m)
            V = sp.Rational(k1, 2) * x1**2 + sp.Rational(k1, 2) * x2**2 + sp.Rational(kc, 2) * (x1 - x2)**2
            H = sp.expand(T + V)
            problems.append(_problem(
                prompt=(
                ...
                H=H, T=T, V=sp.expand(V),
                q_vars=[x1, x2], p_vars=[p1, p2],
                dqdt_exprs=[sp.diff(H, p1), sp.diff(H, p2)],
                dpdt_exprs=[-sp.diff(H, x1), -sp.diff(H, x2)],
                difficulty="tier3", system="coupled_oscillators",
                coordinates="x1, x2",
            ))
    return problems
...
def _tier3_rotating_hoop():
    ...
    for omega in [1, 2, 3]:
        for R in [1, 2]:
            ...
            I = m * R**2
            T = p_theta**2 / (2 * I)
            V = -m * g * R * sp.cos(theta) - sp.Rational(1, 2) * m * omega**2 * R**2 * sp.sin(theta)**2
            H = T + V
            problems.append(_problem(
                prompt=(
    ...
    for kx in [1, 2]:
        for ky in [3, 5]:
            ...
            problems.append(_problem(
                ...
                dqdt_exprs=[sp.diff(H, px), sp.diff(H, py)],
                dpdt_exprs=[-sp.diff(H, x), -sp.diff(H, y)],
                difficulty="tier3", system="2d_oscillator",
                coordinates="x, y",
            ))
    return problems
...
def _tier4_double_pendulum():
    ...
    for m1, m2, L1, L2 in configs:
        ...
        problems.append(_problem(
            ...
            H=H, T=T, V=V,
            q_vars=[theta1, theta2], p_vars=[p1, p2],
            dqdt_exprs=[sp.diff(H, p1), sp.diff(H, p2)],
            dpdt_exprs=[-sp.diff(H, theta1), -sp.diff(H, theta2)],
            difficulty="tier4", system="double_pendulum",
    ...
    for a in [1, 3, 5]:
        for c in [2, 4]:
            ...
            problems.append(_problem(
                ...
                difficulty="tier4", system="legendre_transform",
                coordinates="q",
            ))
    ...
    for b_val in [1, 2]:
        for k_val in [2, 4]:
            ...
            problems.append(_problem(
                ...
                dqdt_exprs=[sp.diff(H, p)],
                dpdt_exprs=[-sp.diff(H, q)],
                difficulty="tier4", system="velocity_dependent_L",
                coordinates="q",
            ))
    return problems
...
def _edge_cases():
    ...
    for k in [1000, 10000]:
        ...
        problems.append(_problem(
            ...
            dqdt_exprs=[sp.diff(H, p)],
            dpdt_exprs=[-sp.diff(H, x)],
            difficulty="edge", system="stiff_spring",
            coordinates="x",
        ))
    ...
    for m in [1, 5]:
        ...
        problems.append(_problem(
            ...
            dqdt_exprs=[sp.diff(H, p)],
            dpdt_exprs=[-sp.diff(H, x)],
            difficulty="edge", system="free_particle",
            coordinates="x",
        ))
    ...
    problems.append(_problem(
        ...
        dqdt_exprs=[sp.diff(H, p)],
        dpdt_exprs=[-sp.diff(H, x)],
        difficulty="edge", system="quartic_only",
        coordinates="x",
    ))
...
def _make_problems():
    ...
    generators = [
        ...
        _tier2_anharmonic, _tier2_morse,
        _tier3_coupled_oscillators, _tier3_magnetic_field,
        _tier3_rotating_hoop, _tier3_2d_oscillator,
        _tier4_double_pendulum, _tier4_lagrangian_to_hamiltonian,
        _edge_cases,
    ]
