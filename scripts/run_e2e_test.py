...
from qgre.generation import UnslothBackend, GenerationOutput
...
def write_step_table_html(html_file, step_num: int, prompts: list[str], completions: list[str],
                          reward_results: list[RewardResult], step_qualities: dict,
                          token_counts: list[int] | None = None):
    """Write one step's results as an HTML table — full text, no truncation."""
    import html as html_mod

    html_file.write(f'<h2>Step {step_num}</h2>\n')
    html_file.write('<table border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse; width:100%; table-layout:fixed; word-wrap:break-word;">\n')
    ...
    # Rows
    for i, (prompt, completion, rr) in enumerate(zip(prompts, completions, reward_results)):
        ...
        html_file.write(f'<td style="white-space:pre-wrap; font-size:12px;">{html_mod.escape(completion)}</td>')
        ...
        for sn in sorted(step_qualities.keys()):
            ...
            html_file.write(f'<td style="text-align:center; color:{color}; font-weight:bold;" title="{tooltip}">{step_mean:.3f}</td>')

        overall_color = "#2d7" if rr.reward > 0.5 else "#da2" if rr.reward > 0.2 else "#d44"
        html_file.write(f'<td style="text-align:center; color:{overall_color}; font-weight:bold;">{rr.reward:.3f}</td>')
...
def main():
    ...
    html_file.write('<style>body{font-family:monospace;margin:20px;background:#1a1a1a;color:#eee;} ')
    ...
    html_file.write('h1{color:#6cf;} h2{color:#9cf;border-bottom:1px solid #555;padding-bottom:5px;} ')
    html_file.write('.metric{display:inline-block;margin:0 15px 5px 0;padding:4px 10px;background:#2a2a2a;border-radius:4px;} ')
    html_file.write('</style></head><body>\n')
    html_file.write(f'<h1>QGRE Engine — E2E Test Results</h1>\n')
    html_file.write(f'<p>Model: {config.model.path} | Mode: {config.algorithm.mode} | ')
    html_file.write(f'Max tokens: {config.generation.max_tokens} | Batch: {len(PROMPTS)} prompts</p>\n')
    ...
    for step in range(10):
        ...
        write_step_table_html(html_file, step + 1, PROMPTS, gen_output.texts,
                              reward_results, step_qualities, token_counts)
        ...
        html_file.write(f'<span class="metric">Reward: {metrics.get("reward/mean", 0.0):.3f}</span>')
        html_file.write(f'<span class="metric">Phase: {game_state.phase}</span>')
        for sn in sorted(step_qualities.keys()):
            html_file.write(f'<span class="metric">Mastery S{sn}: {game_state.get_step_mastery(sn):.3f}</span>')
        html_file.write(f'<span class="metric">VRAM: {vram_now:.2f}/{vram_peak:.2f} GB</span>')
        html_file.write('</div>\n')

        print(f"  loss={metrics.get('loss', 0.0):.6f} reward={metrics.get('reward/mean', 0.0):.3f} "
